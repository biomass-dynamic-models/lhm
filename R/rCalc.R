#' @title Calculate the intrinsic growth rate
#' @description
...
#' @importFrom stats dlnorm sd uniroot
#' @examples
...
setMethod("rCalc", signature = "lhm", function(.Object) {
  ...
  # assign
  rPrior@lognormal_par <- list('E[log(x)]' = mu, 'SD[log(x)]' = sigma, 'E[x]' = theta, 'VAR[x]' = nu, 'CV[x]' = cv)
